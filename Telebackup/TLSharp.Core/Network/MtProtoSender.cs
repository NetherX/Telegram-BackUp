...
namespace TLSharp.Core.Network
{
	...
	{
		//ulong sessionId = GenerateRandomUlong();
		...
		async Task Send(byte[] packet, MTProtoRequest request)
		{
			request.MessageId = _session.GetNewMessageId();
		
			byte[] msgKey;
			byte[] ciphertext;
			using (MemoryStream plaintextPacket =
                makeMemory(32 + packet.Length)) // 8 + 8 + 8 + 4 + 4
            {
		...
		Tuple<byte[], ulong, int> DecodeMessage(byte[] body)
		{
			byte[] message;
		...
		bool processMessage(ulong messageId, int sequence, BinaryReader messageReader, MTProtoRequest request)
		{
			// TODO: check salt
		...
		bool HandleUpdate(ulong messageId, int sequence, BinaryReader messageReader)
		{
			return false;
		...
		bool HandleGzipPacked(ulong messageId, int sequence, BinaryReader messageReader, MTProtoRequest request)
		{
			uint code = messageReader.ReadUInt32();
		...
		bool HandleMsgDetailedInfo(ulong messageId, int sequence, BinaryReader messageReader)
		{
			return false;
		}

		bool HandleBadMsgNotification(ulong messageId, int sequence, BinaryReader messageReader)
		{
			uint code = messageReader.ReadUInt32();
			ulong requestId = messageReader.ReadUInt64();
		...
		bool HandleBadServerSalt(ulong messageId, int sequence, BinaryReader messageReader, MTProtoRequest request)
		{
			uint code = messageReader.ReadUInt32();
		...
		bool HandleMsgsAck(ulong messageId, int sequence, BinaryReader messageReader)
		{
			return false;
		}

		bool HandleNewSessionCreated(ulong messageId, int sequence, BinaryReader messageReader)
		{
			return false;
		}

		bool HandleFutureSalts(ulong messageId, int sequence, BinaryReader messageReader)
		{
			uint code = messageReader.ReadUInt32();
			ulong requestId = messageReader.ReadUInt64();
		...
		bool HandlePong(ulong messageId, int sequence, BinaryReader messageReader)
		{
			return false;
		}

		bool HandlePing(ulong messageId, int sequence, BinaryReader messageReader)
		{
			return false;
		}

		bool HandleContainer(ulong messageId, int sequence, BinaryReader messageReader, MTProtoRequest request)
		{
			uint code = messageReader.ReadUInt32();
		...
		MemoryStream makeMemory(int len)
		{
        ...
        // TODO useme?
        //bool working;
        //async Task WaitWorkEnd()
        //{ if (working)
        //    await Task.Factory.StartNew(() => { while (working) Thread.Sleep(20); }); }
	}
}